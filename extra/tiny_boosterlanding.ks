...
local lock acceleration to (9.81 - thrust/ship:mass)-(ship:airspeed/terminalVelocity)*9.81.
local lock suicideAlt to ship:airspeed*burntime+acceleration*burntime*burntime/2.// kinematics equation solved for d.
until ship:altitude- max(0, ship:geoposition:terrainheight)-landedHeight < suicideAlt {
   print "SuicideAlt: "+suicideAlt at(0, 5).
   print "Adjusted alt: "+(ship:altitude- max(0, ship:geoposition:terrainheight)-landedHeight) at(0, 6).
   set terminalVelocity to ship:airspeed.
}
lock throttle to suicideAlt/(ship:altitude- max(0, ship:geoposition:terrainheight)-landedHeight). 
legs on.
until ship:status = "LANDED" {
   if ship:altitude- max(0, ship:geoposition:terrainheight)-landedHeight > 1 {
      set minThrott to min(minThrott, throttle).
      print "Throttle: "+throttle at(0, 9).
      print "Minimum Throttle: "+minThrott at(0, 10).
      print "Terrain height: "+ship:geoposition:terrainheight at(0, 11).
   }
}
lock throttle to 0.